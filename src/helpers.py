...
class NFXRuntime:
    ...
    def print(self, msg, status="step", color="white", bold=False):
        ...
        msg = "<NO MSG>" if msg is None else msg
    ...
    def run(self, data: str):
        try:
            for raw in data.splitlines():
                ...
                # function+code toggle
                if line.startswith("$!"):
                    self.funcs_allowed = not self.funcs_allowed
                    continue
...
def atomic_fs_aquire_lock(cmd: str, args:list, flags:list) -> int:
    try:
...
$= PID {str(os.getpid())}
$= CMD "nfx {cmd} {" ".join(args)} {" ".join("--"+v for v in flags)}"

$!

$& step_desc
$# print "Lockfile locked by pid=$(PID)",color=red,bold=true
$# print "Command: $(CMD)",color=red,bold=false
...
""".encode("utf-8")
        ...
        if "lockfile-info" not in flags: return None
        try:
            data = ""
            with open(LOCK_FILE_DEF, "r") as f:
                data = f.read()
            vars = {}
            funcs = {}
            inbuilt_funcs = {
            ...
            error("Failed to read Lock File for NFXScript-VM")
            error(str(e), type=step_desc)
            return None
